...
Cómo ya se discutió durante el tema de Estimación, porlo general no se conoce la varinaza de las poblaciones compradas y por lo tanto se usa la exprecion para estimar la varianza a partir de las muestras usando $s^2$.  
...
Para una prueba de dos lados, se rechaza $H_0$ si el valor estimato de $t'$ es mayor que el valor crítico de la ecuación anterior para $t'$ o menor que el valor negativo.  
...
*Respuesta* Si la cirugía incrementa la función biliar, esto es incrementa GBEL, $\mu_{POSTOP} > \mu_{PREOP}$, es decir queremos demostrar que la diferencia en GBEL es positiva. Por lo tanto, la hipótesis nula es que $H_0: \mu_{POSTOP - PREOP} \leq 0$ y por lo tanto $H_A: \mu_{POSTOP - PREOP} > 0$
